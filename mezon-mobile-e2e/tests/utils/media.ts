...
import { ManageDrive } from '../manage-drive/index.js';

function isRealIOS(d: Browser) {
  const caps = (d as any).capabilities || {};
  return /ios/i.test(String(caps.platformName ?? caps.platform ?? '')) || d.isIOS === true;
}
...
export class Media {
  ...
  static async selectImageInNativePicker(filename: string): Promise<void> {
    ...
    try {
      const fileEntry = await $(`android=new UiSelector().textContains("${filename}")`);
