...
const CLAN_PANEL_ITEM = generateE2eSelector('clan_page.header.modal_panel.item');
...
export class ClanMenuPanel extends BasePage {
  ...
  readonly buttons = {
    createCategory: this.page.locator(CLAN_PANEL_ITEM, {
    ...
    clanSettings: this.page.locator(CLAN_PANEL_ITEM, {
      hasText: 'Clan Settings',
    }),
    invitePeople: this.page.locator(CLAN_PANEL_ITEM, {
  ...
  async closeCreateCategoryModal(): Promise<void> {
    ...
    await expect(this.input.categoryName).toBeHidden({ timeout: 5000 });
  }
