...
const PERSISTED_STATE = '{"version":-1,"rehydrated":true}';
const RETAINABLE_ARTIFACT_MODES = ['off', 'on', 'retain-on-failure', 'on-first-retry'] as const;

const getEnvironmentOption = <T extends string>(
  value: string | undefined,
  allowedValues: readonly T[],
  fallback: T
): T => (value && allowedValues.includes(value as T) ? (value as T) : fallback);
...
export const persistentAuthConfigs = {
  account1: {
    ...
    isLogin: 'true',
    isRegistering: NOT_LOADED,
    loadingStatusEmail: NOT_LOADED,
    redirectUrl: 'null',
...
type SessionStoragePayload = {
  mezonSession: { host: string; port: string; ssl: boolean };
  persistAuth: (typeof persistentAuthConfigs)[AccountKey];
};
...
export const updateSessionLocalStorage = async (page: Page, accountKey: AccountKey) => {
  ...
  try {
    const authConfig = getAuthConfig(accountKey);
    ...
    await page.evaluate(
      (authData: SessionStoragePayload) => {
...
function getEnvironmentConfig(): EnvironmentConfig {
  ...
  const baseConfig: EnvironmentConfig = {
    ...
    screenshots: {
      mode: getEnvironmentOption(
        process.env.SCREENSHOT_MODE,
        ['off', 'only-on-failure', 'on'] as const,
        'only-on-failure'
      ),
    ...
    video: {
      mode: getEnvironmentOption(
        process.env.VIDEO_MODE,
        RETAINABLE_ARTIFACT_MODES,
        'retain-on-failure'
      ),
