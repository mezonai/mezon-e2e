...
Then('the underlying modal should remain open', async ({ page }) => {
  ...
  if (modalCount === 0) {
    ...
    console.log('✅ PASS: Some modals remain open after ESC, indicating proper one-by-one behavior');
  }
});
...
Then('I should return to the previous interface level', async ({ page }) => {
  ...
  const isInClanContext = currentUrl.includes('clan') || 
                         await page.getByText(/clan|settings/i).count() > 0;
...
When('I open the sticker upload modal', async ({ page }) => {
  ...
  await page.getByText(/image sticker/i).first().click();
  await page.waitForTimeout(500);
...
When('I press ESC key multiple times', async ({ page }) => {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    const hasVisibleModal = await page.locator('.modal, .dialog, [role="dialog"]').first().isVisible().catch(() => false);
    if (!hasVisibleModal) {
