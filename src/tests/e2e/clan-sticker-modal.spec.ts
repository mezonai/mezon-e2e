...
test.describe('Clan Sticker Modal ESC Key Behavior', () => {
  ...
  test('should close modals one by one when pressing ESC key', async ({ page }) => {
    ...
    await test.step('Test ESC key behavior - should close modal one by one', async () => {
      ...
      console.log(`   Initial "Write your thoughts" input visible: ${escResult.mentionInputTest.initialInputVisible}`);
      console.log(`   Final "Write your thoughts" input visible: ${escResult.mentionInputTest.finalInputVisible}`);
      ...
      escResult.escPressResults.forEach(result => {
        const modalBehavior = result.closedCount === 0 ? 'No effect' :
                             result.closedCount === 1 ? 'Correct (1 modal)' :
                             `BUG (${result.closedCount} modals)`;
        const inputBehavior = result.inputTriggered ? 'INPUT!' : 'No input';
        console.log(`   Press ${result.pressNumber}: ${result.modalsBefore} → ${result.modalsAfter} [${modalBehavior}] [${inputBehavior}]`);
      ...
      if (escResult.closedAllAtOnce) {
        ...
        if (problematicPress) {
          console.log(`   Problematic press #${problematicPress.pressNumber}: closed ${problematicPress.closedCount} modals`);
        }
        issues.push('Modal stacking bug');
        
        await page.screenshot({ path: 'bug-report-esc-closes-all-modals.png', fullPage: true });
        
      } else if (escResult.initialModalCount > 0 && escResult.finalModalCount < escResult.initialModalCount) {
        console.log('\n✅ PASS: ESC key behavior appears correct - modals closed progressively');
        

        const allSingleCloses = escResult.escPressResults.every(press => 
          press.closedCount === 0 || press.closedCount === 1
        );
        ...
        const hadErrors = escResult.escPressResults.some(press => press.modalsAfter > press.modalsBefore);
        if (hadErrors) {
        ...
        console.log(`   Initial: ${escResult.initialModalCount}, Final: ${escResult.finalModalCount}`);
      }
      ...
      if (inputIssues.length > 0) {
        ...
        inputIssues.forEach(press => {
          console.log(`   ❌ Press ${press.pressNumber}: Input ${press.mentionInputBefore} → ${press.mentionInputAfter}`);
        });
      }
      ...
      console.log(`   Bottom Chat Input Test: ${escResult.mentionInputTest.testResult} (triggered: ${escResult.mentionInputTest.escTriggeredInput})`);
            ...
            if (escResult.mentionInputTest.finalInputVisible) {
              ...
              expect(escResult.mentionInputTest.finalInputVisible, 
                'ESC key caused "out ra ngoài" - bottom chat input appeared after pressing ESC').toBe(false);
            } else {
  ...
  test('should verify modal stack behavior with multiple ESC presses', async ({ page }) => {
    ...
    await test.step('Setup multiple modals scenario', async () => {
      ...
      console.log('ℹ️ Testing with available modals - extend this when multiple nested modals are reliably available');
    });
    ...
    await test.step('Test multiple ESC presses', async () => {
      ...
      while (escPressCount < maxEscPresses) {
        ...
        if (modalCountBefore > 1 && modalCountAfter === 0) {
          ...
          console.log(`Expected: Close only 1 modal (${modalCountBefore} -> ${modalCountBefore - 1})`);
