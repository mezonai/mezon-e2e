...
test.describe('Onboarding Guide - Task Completion, Settings, and Preview', () => {
  ...
  const CLAN_SETTING_TAG = 'clan-setting';
  const ENABLE_ONBOARDING_ON_CLAN_STEP = 'Enable onboarding on clan';
  const ENABLE_ONBOARDING_STEP = 'Enable onboarding';
  const VERIFY_ONBOARDING_ENABLED_STEP = 'Verify onboarding is enable';
  ...
  `);
    ...
    await AllureReporter.addLabels({
      tag: [CLAN_SETTING_TAG, 'onboarding', 'pre-question', 'enable'],
    });
    ...
    await AllureReporter.step(ENABLE_ONBOARDING_ON_CLAN_STEP, async () => {
    ...
    await AllureReporter.step(ENABLE_ONBOARDING_STEP, async () => {
    ...
    await AllureReporter.step(VERIFY_ONBOARDING_ENABLED_STEP, async () => {
      await onboardingPage.verifyEnableOnboardingOnClanSettingsSidebar();
      await onboardingPage.clickDisableOnboarding();
  ...
  `);
    ...
    await AllureReporter.addLabels({
      tag: [CLAN_SETTING_TAG, 'onboarding', 'task'],
    });
  ...
  `);
    ...
    await AllureReporter.addLabels({
      tag: [CLAN_SETTING_TAG, 'onboarding', 'task', 'preview-mode'],
    });
