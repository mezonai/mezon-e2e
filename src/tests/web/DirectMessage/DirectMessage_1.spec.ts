...
test.describe('Direct Messages - Invoice Status, Conversation Pins, and Group Calls', () => {
  const DIRECT_MESSAGE_TAG = 'direct-message';
  const VERIFY_PINNED_CONVERSATION_STEP = 'Verify pinned conversation is visible on pin list';
  ...
  test('Display invoice status in DM list and header', async ({ page }) => {
    ...
    await AllureReporter.addLabels({
      tag: [DIRECT_MESSAGE_TAG, 'invoice-status', 'voice-status'],
    });
  ...
  test('Pin Conversation on direct message list', async ({ page }) => {
    ...
    await AllureReporter.addLabels({
      tag: [DIRECT_MESSAGE_TAG, 'pin-conversation'],
    });
    ...
    await AllureReporter.step(VERIFY_PINNED_CONVERSATION_STEP, async () => {
