...
test.describe('Timeline - Creation, Updates, Attachments, and Calendar Events', () => {
  const CREATE_FIRST_EVENT_STEP = 'Create First event';
  const VERIFY_EVENT_VISIBLE_STEP = 'Verify event is visble on tab';
  ...
  test('Verify that user can create a timeline', async ({ page }) => {
    ...
    await AllureReporter.step(CREATE_FIRST_EVENT_STEP, async () => {
      const dateData = await messagePage.fillTitleAndDescription(data);
      await messagePage.clickSave();
      date = dateData;
    });

    await AllureReporter.step(VERIFY_EVENT_VISIBLE_STEP, async () => {
