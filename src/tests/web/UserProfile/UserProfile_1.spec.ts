...
test.describe('User Profile - Avatar Visibility Across Direct Message Views', () => {
  ...
  test.describe('Owner Account - Avatar Visibility Across Direct Message Views', () => {
    ...
    test('Validate footer avatar', async ({ page }) => {
      ...
      await profilePage.navigate(ROUTES.DIRECT_FRIENDS);
      const footerAvatar = await messagePage.getFooterAvatar();
  ...
  test.describe('Friend Account - Avatar Visibility Across Direct Message Views', () => {
    ...
    test('TC01: Friend List _ avatar ', async ({ page }) => {
      ...
      await expect(friendItem).toBeVisible({ timeout: 15000 });

      const accountAvatar = friendItem.locator(AVATAR_IMAGE_SELECTOR);
    ...
    test('TC02: Direct Message _ Conversation List _ Avatar user', async ({ page }) => {
      ...
      const accountAvatar = DmItem.locator(AVATAR_IMAGE_SELECTOR);
    ...
    test('TC04: Direct Message _ Dual Chat _ Display name', async ({ page }) => {
      ...
      const accountAvatar = messageItemWithProfileName.locator(AVATAR_IMAGE_SELECTOR);
      await expect(accountAvatar).toBeVisible({ timeout: 5000 });
      const accountSrc = await accountAvatar.getAttribute('src');
      const avatarId = getImageId(accountSrc);
      expect(profileId).not.toBeNull();
