...
test.describe('User Settings - Clan Identity and User Profile Updates', () => {
  const NAVIGATE_PROFILE_STEP = 'Navigate to profile tab';
  const USER_PROFILE_TAG = 'user-profile';
  const CLAN_PROFILE_TAG = 'clan-profile';
  ...
  test('Change avatar clan - button visible', async ({ page }) => {
    ...
    await AllureReporter.step(NAVIGATE_PROFILE_STEP, async () => {
    ...
    await AllureReporter.addLabels({
      tag: [USER_PROFILE_TAG, 'avatar-change', 'ui-visibility', CLAN_PROFILE_TAG],
    });
  ...
  test('Change clan nickname', async ({ page }) => {
    ...
    await AllureReporter.addLabels({
      tag: [USER_PROFILE_TAG, 'nickname-change', 'profile-update', CLAN_PROFILE_TAG],
    });
