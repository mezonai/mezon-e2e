...
test.describe('Unread State - Mark Clans, Channels, and Direct Messages as Read', () => {
  ...
  const MARK_AS_READ_TAG = 'mark-as-read';
  ...
  const CREATE_CLAN_STEP = 'User A creates a clan';
  const INVITE_TO_CLAN_STEP = 'User A invite user B to clan and user B accept it';
  const MARK_AS_READ_STEP = 'User B click mark as read';
  const CLEANUP_CLAN_STEP = 'Cleanup clan';
  const VERIFY_NO_CHANNEL_HIGHLIGHT_STEP =
  ...
  test('Verify that I can mark as read on clan (badge from mention name) ', async ({ dual }) => {
    ...
    await AllureReporter.addLabels({
      tag: [CLAN_TAG, TEXT_CHANNEL_TAG, 'channel-badge', 'header-inbox', MARK_AS_READ_TAG],
    });
    ...
    await AllureReporter.step(CREATE_CLAN_STEP, async () => {
      await clanFactory.setupClan(ClanSetupHelper.configs.markAsRead, pageA);
    });

    await AllureReporter.step(INVITE_TO_CLAN_STEP, async () => {
    ...
    await AllureReporter.step(MARK_AS_READ_STEP, async () => {
    ...
    await AllureReporter.step(CLEANUP_CLAN_STEP, async () => {
  ...
  test('Verify that I can mark as read on clan (highlight on channel name)', async ({ dual }) => {
    ...
    await AllureReporter.addLabels({
      tag: ['channel-highlight', TEXT_CHANNEL_TAG, CLAN_TAG, MARK_AS_READ_TAG],
    });
    ...
    await AllureReporter.step(
      ...
      async () => {
        for (const name of channelNames) {
          ...
          const isNewChannelPresent = await clanPageA.isNewChannelPresent(name);
          expect(isNewChannelPresent).toBe(true);
          await messageHelperA.sendTextMessage(TEXT_MESSAGE);
        }
      }
    );
    ...
    await AllureReporter.step(VERIFY_NO_CHANNEL_HIGHLIGHT_STEP, async () => {
      for (const name of channelNames) {
  ...
  test('Verify that I can mark as read on channel (highlight)', async ({ dual }) => {
    ...
    await AllureReporter.addLabels({
      tag: [TEXT_CHANNEL_TAG, MARK_AS_READ_TAG, 'channel-highlight'],
    });
  ...
  test('Verify that I can mark as read on channel (mention)', async ({ dual }) => {
    ...
    await AllureReporter.addLabels({
      tag: [TEXT_CHANNEL_TAG, MARK_AS_READ_TAG, 'channel-mentions'],
    });
  ...
  test('Verify that I can mark as read on direct message (highlight)', async ({ dual }) => {
    ...
    await AllureReporter.addLabels({
      tag: ['direct-message', MARK_AS_READ_TAG, 'highlight'],
    });
