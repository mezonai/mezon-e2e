...
import { CHANNEL_MESSAGE_STEPS, CHANNEL_MESSAGE_TAGS } from './ChannelMessageTestConstants';
...
test.describe('Channel Messages - Topic Restrictions and Shared Contacts', () => {
  ...
  test('Verify that user is banned can not send message on topic', async ({ dual }) => {
    ...
    await AllureReporter.addLabels({
      tag: [CHANNEL_MESSAGE_TAGS.clan, 'ban-user', 'topic', 'send-message'],
    });
    ...
    await AllureReporter.step(CHANNEL_MESSAGE_STEPS.acceptFriendRequest, async () => {
    ...
    await AllureReporter.step(CHANNEL_MESSAGE_STEPS.createClan, async () => {
      await clanFactory.setupClan(ClanSetupHelper.configs.channelMessage3, pageA);
    });

    await AllureReporter.step(CHANNEL_MESSAGE_STEPS.inviteToClan, async () => {
      await clanPageA.clickButtonInvitePeopleFromMenu();
      const url = await clanPageA.inviteUserToClanByUsername(userNameB);
      await clanPageB.joinClanByUrlInvite(url);
    });

    await AllureReporter.step(CHANNEL_MESSAGE_STEPS.banUserInChannel, async () => {
      await pageA.reload();
      await clanPageA.openMemberList();
      await clanPageA.banUserByName(userNameB);
    });

    await AllureReporter.step(CHANNEL_MESSAGE_STEPS.sendMessageAndCreateTopic, async () => {
    ...
    await AllureReporter.step(CHANNEL_MESSAGE_STEPS.cleanupClan, async () => {
  ...
  test('Send message from short profile in clan channel', async ({ dual }) => {
    ...
    await AllureReporter.addLabels({
      tag: [CHANNEL_MESSAGE_TAGS.clan, 'short-profile', 'send-message'],
    });
  ...
  test('Verify that user can share friend contact card in clan channel', async ({ dual }) => {
    ...
    await AllureReporter.addLabels({
      tag: [
        CHANNEL_MESSAGE_TAGS.clan,
        CHANNEL_MESSAGE_TAGS.contactCard,
        CHANNEL_MESSAGE_TAGS.share,
      ],
    });
    const clanPageA = new ClanPage(pageA);
    const clanPageB = new ClanPage(pageB);
