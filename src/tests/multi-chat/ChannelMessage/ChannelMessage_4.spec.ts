...
import {
  ...
  CHANNEL_MESSAGE_STEPS,
  CHANNEL_MESSAGE_TAGS,
} from './ChannelMessageTestConstants';
...
test.describe('Channel Messages - Shared Contacts, Voice Links, and Welcome Messages', () => {
  ...
  test('Verify that user can share contact in group', async ({ dual }) => {
    ...
    await AllureReporter.addLabels({
      tag: ['group', CHANNEL_MESSAGE_TAGS.contactCard, CHANNEL_MESSAGE_TAGS.share],
    });
    ...
    await AllureReporter.step(
      ...
      async () => {
        ...
        await messagePageA.sendMessageWhenInDM(CHANNEL_MESSAGE_DATA.messageToForward);
  ...
  test('Verify that user can share contact from short profile', async ({ dual }) => {
    ...
    await AllureReporter.addLabels({
      tag: ['group', CHANNEL_MESSAGE_TAGS.contactCard, CHANNEL_MESSAGE_TAGS.share, 'short-profile'],
    });
    const messageHelperB = new MessageTestHelpers(pageB);
    await AllureReporter.step(CLEANUP_STEP_NAME, async () => {
      await Promise.allSettled([
