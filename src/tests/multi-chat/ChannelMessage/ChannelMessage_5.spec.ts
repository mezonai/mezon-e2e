...
import { CHANNEL_MESSAGE_DATA, CHANNEL_MESSAGE_STEPS } from './ChannelMessageTestConstants';
...
test.describe('Channel Messages - Profile Status and Blocked User Actions', () => {
  ...
  test('Verify that profile status is displayed correctly in the channel member list', async ({
    ...
    await AllureReporter.step(CHANNEL_MESSAGE_DATA.profileStatus.step, async () => {
      await profilePageA.openFooterProfileModal();
      await profilePageA.openSelectProfileStatusModal();

      await profilePageA.setProfileStatus(
        CHANNEL_MESSAGE_DATA.profileStatus.name,
        CHANNEL_MESSAGE_DATA.profileStatus.duration
      );
    });
  ...
  test('Verify that user cannot forward message for another one who has been blocked', async ({
    ...
    await test.step(CHANNEL_MESSAGE_STEPS.openDirectMessage, async () => {
  ...
  test('Verify that User A cannot send a friend request after blocking User B', async ({
    ...
    await test.step('User A blocks User B from the DM', async () => {
      await friendPageB.page.waitForTimeout(2000);
