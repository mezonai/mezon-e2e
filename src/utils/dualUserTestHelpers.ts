...
export class DualUserTestHelpers {
  ...
  async findAndEditMessage(uniqueMessage: string, editedMessage: string) {
    ...
    for (let i = 0; i < messageCount; i++) {
      ...
      if (
        msgText.includes(uniqueMessage) &&
        ...
        try {
          ...
          const contextMenu = this.page
            .locator('[role="menu"], .context-menu, [class*="context"], [class*="menu"]')
            .first();
  ...
  private async editMessageFromContextMenu(editedMessage: string, uniqueMessage: string) {
    ...
    try {
      ...
      const editButton = this.page
        .locator(
          'button:has-text("Edit Message"), button:has-text("Edit"), [data-testid="edit-button"], [class*="edit"]'
        )
        .first();
  ...
  private async findEditInput(uniqueMessage: string) {
    const editInputSelectors = ['div[contenteditable="true"]', 'textarea', 'input[type="text"]'];
