...
export class DualUserTestHelpers {
	...
	async loginWithEmail(email: string, otp?: string, isAutoOtp = false) {
		...
		const emailInput = this.page.locator('input[type="email"], input[placeholder*="email" i], input[name*="email" i]').first();
		await emailInput.waitFor({ state: 'visible', timeout: 15000 });
		await emailInput.fill(email);

		const sendOtp = this.page.locator('button:has-text("Send OTP"), button:has-text("Continue"), button[type="submit"]').first();
		if (await sendOtp.isVisible({ timeout: 5000 })) {
			await sendOtp.click();
		}

		if (isAutoOtp && otp) {
			const otpInput = this.page.locator('input[placeholder*="otp" i], input[placeholder*="code" i], input[maxlength="6"]').first();
	...
	async sendMessageWithMention(uniqueMessage: string, mentionUser: string = 'kien.trinh') {
		const input = this.page.locator('textarea#editorReactMentionChannel.mentions_input, textarea.mentions_input, div[contenteditable="true"]').first();
	...
	private async selectUserFromMentionDropdown(dropdown: any, username: string) {
		const mentionOptions = dropdown.locator('[role="option"], [role="menuitem"], li, div, [class*="item"], [class*="option"]');
		...
		for (let i = 0; i < mentionCount; i++) {
			...
			if (optionText.toLowerCase().includes(username.toLowerCase()) || optionText.toLowerCase().includes(username.split('.')[0])) {
				await option.click();
				await this.page.waitForTimeout(1000);
				return true;
			}
	...
	async findAndReplyToMessage(uniqueMessage: string, replyText: string = 'okie tao đã rep') {
		const messages = this.page.locator('[data-testid="message"], .message, .chat-message, div[class*="message"]');
	...
	async findAndEditMessage(uniqueMessage: string, editedMessage: string) {
		const messages = this.page.locator('[data-testid="message"], .message, .chat-message, div[class*="message"]');
		const messageCount = await messages.count();
		...
		for (let i = 0; i < messageCount; i++) {
			...
			if (msgText.includes(uniqueMessage) && 
				!msgText.includes('okie tao đã rep') && 
				!msgText.includes('reply') &&
				!msgText.includes('Reply')) {
				...
				try {
					...
					const contextMenu = this.page.locator('[role="menu"], .context-menu, [class*="context"], [class*="menu"]').first();
					if (await contextMenu.isVisible({ timeout: 3000 })) {
						return await this.editMessageFromContextMenu(editedMessage, uniqueMessage);
					}
				} catch (error) {
					continue;
				}
	...
	private async editMessageFromContextMenu(editedMessage: string, uniqueMessage: string) {
		const messageHelpers = new MessageTestHelpers(this.page);
		
		try {
			const editButton = await messageHelpers.findEditOption();
			await editButton.click();
			await this.page.waitForTimeout(3000);
			
			const editInput = await this.findEditInput(uniqueMessage);
			if (editInput) {
				return await this.performEdit(editInput, editedMessage);
			}
		} catch (editError: any) {
			// Fallback to manual edit button
			const editButton = this.page.locator('button:has-text("Edit Message"), button:has-text("Edit"), [data-testid="edit-button"], [class*="edit"]').first();
	...
	private async findEditInput(uniqueMessage: string) {
				...
				if (await element.isVisible({ timeout: 1000 })) {
					...
					try {
						if (selector.includes('contenteditable')) {
							...
							currentText = await element.inputValue() || '';
						}
					} catch (e) {
						continue;
					}
					
					if (currentText.includes(uniqueMessage) || currentText.includes('@kien.trinh')) {
						return element;
					}
				}
			}
	...
	private async performEdit(editInput: any, editedMessage: string) {
	...
	async openInboxMentions() {
		const inboxButton = this.page.locator('button[title="Inbox"], button[aria-label*="Inbox" i], [data-testid="inbox-button"]').first();
		...
		const mentionsTab = this.page.locator('button:has-text("Mentions"), [role="tab"]:has-text("Mentions"), .mentions-tab').first();
		...
		if (await mentionsTab.isVisible({ timeout: 5000 })) {
			...
			const fallbackMentions = this.page.locator('button:has-text("Mentions"), a:has-text("Mentions"), [data-testid="mentions-tab"]').first();
